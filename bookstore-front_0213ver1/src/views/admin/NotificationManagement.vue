    ...
    <div class="main-content">
      ...
      <div class="card">
        ...
        <div class="input-grid">
          <input v-model="newNotification.notificationTypeID" type="number" placeholder="通知類型ID"
            class="input short-input" />
          <input v-model="newNotification.customerID" type="number" placeholder="顧客ID" class="input short-input" />
          <input v-model="newNotification.content" type="text" placeholder="通知內容" class="input" />
      ...
      <div class="card">
        <div class="filter-section">
          <div class="search-box">
            ...
            <input v-model="filterUsername" type="text" placeholder="輸入顧客名稱以篩選"
          ...
          <table>
            ...
            <tbody>
              <tr v-for="notification in notifications" :key="notification.notificationID">
                ...
                <td>{{ notification.customer.username }}</td> <!-- ✅ 這裡從 customerID 改成 username -->
      ...
      <div v-if="isEditModalOpen" class="modal-overlay">
        <div class="modal no-scroll">
          ...
          <div class="modal-body">
            ...
            <input v-model="editNotification.notificationTypeID" type="number" placeholder="通知類型ID"
              class="input short-input" />
            <!-- 顯示顧客名稱 (唯讀) -->
            <input v-model="editNotification.username" type="text" readonly placeholder="顧客名稱"
              class="input short-input disabled" />
            <input v-model="editNotification.content" type="text" placeholder="通知內容" class="input" />
          </div>
...
export default {
  ...
  methods: {
    ...
    async createNotification() {
      try {
        const payload = {
          ...
          customer: { customerID: this.newNotification.customerID }, // ✅ 只傳 customerID，不傳整個 customer
          content: this.newNotification.content,
        };
        console.log("📢 發送通知請求:", JSON.stringify(payload, null, 2));
        ...
        Swal.fire("錯誤", `無法創建通知，請檢查後端：${error.message}`, "error");
      }
    }
	,
    // 獲取所有通知
    ...
    // 重置篩選
    resetFilter() {
      this.filterUsername = ""; // ✅ 改為清除 `username`
      this.notifications = this.allNotifications; // ✅ 恢復所有通知
    }
    ,
