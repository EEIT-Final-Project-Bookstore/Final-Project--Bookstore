<template>
  <div class="app">
    ...
    <!--0213新增分類頁面-  -------------------------------------- 📌 中文書籍分類移到 Header 底部 -->
  <div class="category-container">
    <nav class="category-nav">
      <ul>
        <li>
          <router-link class="category-link" :to="{ path: '/selected-categories' }">📚 中文</router-link>
        </li>
        <li><router-link to="/foreign">外文</router-link></li>
...
export default {
  ...
  console.log("✅ SearchPage.vue 已載入");
  this.$nextTick(() => {
    const el = document.querySelector('.search-bar-container');
    console.log("🔍 search-bar-container:", el ? "✅ 存在" : "❌ 不存在");
    if (el) {
...
.site-header {
  ...
  align-items: center;
  width: 100%;
  min-height: 300px; /* 或者依需要調整   02133*/
  padding: 10px 0;
...
.header-center {
  ...
  flex-grow: 1;
  height: 60px; /* ✅ 確保 header 內部有足夠的空間 */
}
...
/* 0213新增分類頁面---------------------------------------------------------------------------------------------------------------
📌 修正 category-container 位置，確保它緊貼 header 底部 */
/* 📌 修正 category-container，使其正確對齊 Header 底部 */
.category-container {
  width: 100%;
  background-color: white;
...
.category-nav a {
  text-decoration: none;
  color: #333;
  font-size: 1.5rem; /* ✅ 字體變大 */
  font-weight: bold;
  padding: 15px 30px; /* ✅ 內邊距加大，按鈕更寬更高 */
  transition: color 0.2s ease-in-out, background-color 0.2s ease-in-out;
  border-radius: 8px;
    min-width: 160px; /* ✅ 增加按鈕寬度，確保不會變窄 */
