...
export default {
  ...
  methods: {
    async fetchReviews() {
      try {
        const response = await fetch("http://192.168.23.87:8080/reviews");
    ...
    async deleteReview(reviewID) {
      try {
        const response = await fetch(
          `http://192.168.23.87:8080/reviews/${reviewID}`,
          { method: "DELETE" }
        );
    ...
    async updateReview() {
      try {
        const response = await fetch(
          `http://192.168.23.87:8080/reviews/${this.currentReview.reviewID}`,
          {
            method: "PUT",
