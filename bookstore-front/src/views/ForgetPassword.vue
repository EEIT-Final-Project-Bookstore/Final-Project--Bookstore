...
export default {
  ...
  methods: {
    ...
    async checkUserEmail() {
      ...
      try {
        // 檢查 email 是否存在
        const resEmail = await axiosapi.get(
          `/api/customers/checkEmail?email=${email}`
        );
        ...
        // 檢查 username 是否存在
        const resUser = await axiosapi.get(
    ...
    async resetPassword() {
      ...
      let oldData;
      try {
        const getRes = await axiosapi.get(`/api/customers/${username}`);
      ...
      // 3) 呼叫PUT更新
      try {
        await axiosapi.put(`/api/customers/${username}`, updatedData);
