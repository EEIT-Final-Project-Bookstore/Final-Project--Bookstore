...
//		System.out.println("資料庫中讀取的最終金額: " + order.getFinalAmount());
	    ...
	    //特店交易時間
	    aioCheck.setMerchantTradeDate(order.getOrderCreationTime().format(DateTimeFormatter.ofPattern("yyyy/MM/dd HH:mm:ss"))); // 格式化日期);
	    //交易金額
	    ...
	    //導向前端付款結果
	    aioCheck.setClientBackURL("http://192.168.23.87:5173/orders/paymentResult");
    ...
    public String returnURL(@RequestParam("MerchantTradeNo") String MerchantTradeNo,
        ...
        if (RtnCode == 1) {
            ...
            for (OrderDetailBean detail : orderDetails) {
                BookBean book = detail.getBook();
                int orderedQuantity = detail.getQuantity();
                int currentStock = book.getStock();
                if (currentStock < orderedQuantity) {
                    System.out.println("庫存不足，無法扣庫存: " + book.getBookId());
                    return "0|err";
                }
                book.setStock(currentStock - orderedQuantity);  // 減少庫存
            }
