...
public class BookService {
    ...
    // 依據分類名稱取得書籍列表
   	public List<BookDTO> getBooksByCategory(String categoryName) {
   		List<BookBean> books = bookRepository.findBySubcategory_Category_CategoryName(categoryName);
    ...
    public BookDTO updateBook(Integer id, BookDTO bookDTO) {
        ...
        book.setSubcategory(subcategoryService.getSubcategoryEntityById(bookDTO.getSubcategoryId()));
    ...
    private BookDTO convertToDTO(BookBean book) {
        ...
        dto.setAuthorName(book.getAuthor().getAuthorName()); // 取得名稱
        dto.setAuthorDescription(book.getAuthor().getAuthorDescription());//0217
        dto.setPublisherId(book.getPublisher().getPublisherId());
		dto.setPublisherName(book.getPublisher().getPublisherName()); // 取得名稱
        dto.setLanguageId(book.getLanguage().getLanguageId());
        dto.setSubcategoryId(book.getSubcategory().getSubcategoryId());
        dto.setSubcategoryName(book.getSubcategory().getSubcategoryName()); // 取得子分類名稱
		dto.setCategoryName(book.getSubcategory().getCategory().getCategoryName()); // 取得分類名稱
    ...
    public List<BookDTO> searchBooks(String keyword) {
        List<BookBean> books = bookRepository
            .findByBookNameContainingIgnoreCaseOrAuthor_AuthorNameContainingIgnoreCaseOrPublisher_PublisherNameContainingIgnoreCase(
